...
namespace Bookey\BookeyPaymentGateway\Model;

use Bookey\BookeyPaymentGateway\Helper\Crypto;
use Magento\Payment\Gateway\Response\HandlerInterface;
use Magento\Framework\Exception\LocalizedException;

class BookeyPayment extends \Magento\Payment\Model\Method\AbstractMethod implements HandlerInterface
{
	...
	public function handle(array $handlingSubject, array $response)
	{
		...
		$apiPublicKey = $response['GATEWAY_API_PUBLIC_KEY'];
		...
		$refund_signature = Crypto::generateSignature($refund_details, $apiKey, $apiPublicKey);
		...
		if ($parsed_header['response_code'] == '204') {
			...
			$error_message = 'Bookey refunding error: Failed Signature Check when communicating with the Bookey gateway.';
			...
			$error_message = 'Bookey refunding error with returned message from gateway: '.$return_message.$return_message_explain;
		} else {
			$error_message = "Bookey refunding failed with unknown error.";
		}
